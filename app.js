...
/**
 * Resolve a placeholder like "Winner R2-5"
 * Returns { displayName, logo, clickable }
 * - displayName: string to show
 * - logo: URL or null
 * - clickable: true if a real team name exists and can be clicked
 */
function resolvePlaceholder(name) {
  if (!name) return { displayName: '—', logo: null, clickable: false };
  // If it's an actual team (not a Winner placeholder) return directly
  if (!name.toString().startsWith('Winner R')) {
    return { displayName: name, logo: getTeamLogo(name), clickable: (name !== '—' && !name.toString().startsWith('Seed')) };
  }
  // Parse "Winner R{round}-{index}"
  const m = name.match(/Winner R(\d+)-(\d+)/i);
  if (!m) return { displayName: 'Winner', logo: null, clickable: false };
  const sourceRoundIdx = parseInt(m[1], 10) - 1; // convert to 0-based
  const sourceMatchIdx = parseInt(m[2], 10) - 1;
  if (!rounds[sourceRoundIdx] || !rounds[sourceRoundIdx][sourceMatchIdx]) {
    return { displayName: 'Winner', logo: null, clickable: false };
  }
  const sourceMatch = rounds[sourceRoundIdx][sourceMatchIdx];
  // if the source match has a winner string, show that
  if (sourceMatch.winner) {
    const winner = sourceMatch.winner;
    return { displayName: winner, logo: (sourceMatch.teamA===winner?sourceMatch.logoA:sourceMatch.logoB) || getTeamLogo(winner), clickable: true };
  }
  // else show generic label until it's decided
  return { displayName: 'Winner', logo: null, clickable: false };
}
...
function render(){
  ...
  rounds.forEach((roundMatches, rIdx) => {
    ...
    roundMatches.forEach((m, mIdx) => {
      ...
      // Resolve team A display name/logo
      const resolvedA = resolvePlaceholder(m.teamA);
      const a = document.createElement('div');
      a.className = 'teamBtn' + (m.winner === resolvedA.displayName ? ' picked' : '');
      // Attach clickable only if actual selectable team present
      if (resolvedA.clickable) a.onclick = ()=> handlePick(rIdx,mIdx,resolvedA.displayName);
      else a.style.cursor = 'default';
      
      if (resolvedA.logo){
        const img = document.createElement('img'); 
        img.className='logo'; 
        img.src = resolvedA.logo;
        img.onerror = () => img.style.display = 'none'; // Hide if logo fails
        a.appendChild(img);
      }
      const nameA = document.createElement('div'); 
      nameA.className='name'; 
      nameA.innerText = resolvedA.displayName || '—';
      a.appendChild(nameA);

      // Resolve team B display name/logo
      const resolvedB = resolvePlaceholder(m.teamB);
      const b = document.createElement('div');
      b.className = 'teamBtn' + (m.winner === resolvedB.displayName ? ' picked' : '');
      if (resolvedB.clickable) b.onclick = ()=> handlePick(rIdx,mIdx,resolvedB.displayName);
      else b.style.cursor = 'default';
      
      if (resolvedB.logo){
        const imgb = document.createElement('img'); 
        imgb.className='logo'; 
        imgb.src = resolvedB.logo;
        imgb.onerror = () => imgb.style.display = 'none';
        b.appendChild(imgb);
      }
      const nameB = document.createElement('div'); 
      nameB.className='name'; 
      nameB.innerText = resolvedB.displayName || '—';
      b.appendChild(nameB);
...
function handlePick(roundIdx, matchIdx, winnerName){
  ...
  // find the actual team name inside rounds[roundIdx][matchIdx]
  const matchObj = rounds[roundIdx][matchIdx];
  if (!matchObj) return;

  // winnerName should be a concrete team string (resolvePlaceholder ensures this)
  matchObj.winner = winnerName;

  // downstream clearing & propagation
  if (roundIdx === 0) {
    const match = rounds[0][matchIdx];
    const winnerLogo = (match.teamA===winnerName ? match.logoA : match.logoB);
    // second round expects the winner on teamB slot for r32 entries
...
// Submit bracket to Sheets (GET workaround to avoid CORS)
submitBtn.addEventListener('click', ()=>{
  ...
  fetch(url)
    .then(async res => {
      // try to parse JSON - apps script returns JSON text
      const text = await res.text();
      try {
        return JSON.parse(text);
      } catch(e) {
        throw new Error('Invalid response from sheet: ' + text);
      }
    })
    .then(response => {
